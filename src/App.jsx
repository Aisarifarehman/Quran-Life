...
async function aiTranslateChunk(chunk, langName, surahNum, chunkIndex) {
  ...
  try {
    const r = await fetch("https://quranlife-ai.aisarifarehman.workers.dev", {
      method: "POST",
...
async function askAI(prompt, langName, cacheKey) {
  const r = await fetch("https://quranlife-ai.aisarifarehman.workers.dev", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ type: "askAI", cacheKey, prompt, langName })
  });
...
export default function QuranLife() {
  ...
  const getOrLoad = useCallback(async (key, prompt, force = false) => {
    ...
    try {
      const text = await askAI(prompt, curLang.n, key);
