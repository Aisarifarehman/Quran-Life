...
// ─── AI — QUEUED, RATE-LIMIT SAFE, AUTO-RETRY ─────────────────
// Global request queue — ensures only 1 AI call runs at a time,
// preventing HTTP 429 rate-limit errors from simultaneous requests.
let _aiQueue = Promise.resolve();
let _aiLastCall = 0;
const AI_MIN_GAP_MS = 1200; // minimum 1.2s between calls
...
async function askAI(prompt, langName, retries = 3) {
  ...
  const result = await (_aiQueue = _aiQueue.then(async () => {
    // Enforce minimum gap between successive calls
    const now = Date.now();
    const gap = now - _aiLastCall;
    if (gap < AI_MIN_GAP_MS) await new Promise(res => setTimeout(res, AI_MIN_GAP_MS - gap));
      ...
      try {
        const r = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            messages: [{
              role: "user",
              content: `You are a Quranic scholar. ${prompt}\n\nIMPORTANT: Write your COMPLETE response in ${langName} language only. Plain flowing text only. No JSON. No bullet points. No markdown. Under 180 words.`
            }]
          })
        });
        ...
        if (e.message === "NO_KEY" || e.message === "RATE_LIMIT" || attempt >= retries) throw e;
        // Network error — retry after short wait
        await new Promise(res => setTimeout(res, 1500 * (attempt + 1)));
      }
    }
    throw new Error("RATE_LIMIT"); // should not reach
  }).catch(e => { throw e; }));
  return result;
}
...
export default function QuranLife() {
  ...
  const getOrLoad = useCallback(async (key, prompt, force = false) => {
    ...
    try {
      ...
      let msg;
      if (e.message === "NO_KEY") {
        msg = "AI Knowledge coming soon. The app is fully functional for reading, audio and translation.";
      } else if (e.message === "RATE_LIMIT") {
        msg = "RATE_LIMIT — Tap Retry.";
      } else {
        msg = "Failed to load. Tap Retry.";
      }
  ...
  const loadTabContent = useCallback((verse, tab, force = false) => {
    ...
    // Translation tab: use verse.translation directly from Quran.com API — NO AI needed.
    // This avoids unnecessary AI calls and is always accurate.
    if (tab === "translation") {
  ...
  }, [surahNum, curSurah, lang, cache, getOrLoad, setCache]);
  ...
  const css = `
    *{box-sizing:border-box;margin:0;padding:0;max-width:100%}
    ...
    .ar{font-family:'Amiri','Noto Naskh Arabic',serif!important;word-break:break-word;overflow-wrap:anywhere;white-space:normal;min-width:0}
    ...
    .mushaf-text{font-family:'Amiri','Noto Naskh Arabic',serif;font-size:24px;line-height:3;direction:rtl;text-align:justify;color:#1a0500;word-spacing:4px}
    ...
    .zoom-arabic{touch-action:pan-y pinch-zoom;display:block;width:100%;word-break:break-word;overflow-wrap:anywhere;white-space:normal;min-width:0;max-width:100%;overflow:visible}
      ...
      <div className="fade" style={{ paddingBottom: 80 }}>
        ...
        <div style={{ padding: "10px 12px" }}>
          ...
          {!mushafMode && verses.map(verse => {
            ...
            return (
              <div key={verse.number} className="fade" style={{ background: darkMode ? "#1a1a1a" : "#fff", border: `.5px solid ${sajdah ? "#c9943a66" : bkd ? "#8e44ad44" : isOpen ? G : darkMode ? "#2a2a2a" : "#e2e8e4"}`, borderRadius: 13, marginBottom: 9, overflow: "visible", boxShadow: isOpen ? `0 0 0 2px rgba(15,81,50,.09)` : sajdah ? "0 0 0 2px rgba(201,148,58,.15)" : "none" }}>
                {/* Sajdah Banner */}
                {sajdah && (
                ...
                <div style={{ padding: "14px 14px 12px", minWidth: 0, width: "100%", boxSizing: "border-box" }}>
                  <div className="ar zoom-arabic" style={{ fontSize: fontSize, lineHeight: 2.2, direction: "rtl", textAlign: "right", color: darkMode ? "#e8e8e8" : "#1a1a1a", marginBottom: 10, paddingBottom: 10, borderBottom: `.5px solid ${darkMode ? "#2a2a2a" : "#f4f4f4"}`, wordBreak: "break-word", overflowWrap: "anywhere", whiteSpace: "normal", minWidth: 0, maxWidth: "100%", display: "block" }}>
                    {verse.arabic}
                  </div>
                  {(() => {
                    // Translation is already loaded from Quran.com API in verse.translation
                    // For English and Arabic: direct text is always available
                    // For other languages: AI translation updates verse.translation in background
                    const displayText = verse.translation || "";
                    return (
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 6 }}>
                        ...
                        {displayText && (
                          <button onClick={() => speakInLang(displayText, lang)}
                ...
                {isOpen && (
                  <div style={{ borderTop: ".5px solid rgba(15,81,50,.1)", background: "#fafffe", padding: 13 }}>
                    ...
                    {/* AI content — queued, rate-limit safe, auto-retry */}
                    {(() => {
                      const key = `${surahNum}-${verse.number}-${activeTab}-${lang}`;
                      const entry = cache[key] || {};
                      // Only trigger load if not already loading/loaded/errored
                      if (!entry.text && !entry.loading && !entry.error) {
                        // Use setTimeout(0) to avoid calling state-setter during render
                        setTimeout(() => loadTabContent(verse, activeTab), 0);
                      }
                      const tabLabel = activeTab === "tafsir" ? "Tafsir"
                        : activeTab === "revelation" ? "Revelation History"
                        : activeTab === "science" ? "Scientific Connection"
                        : "Hadith & Authenticity";
                      return (
                        <div>
                          ...
                          {entry.error && (
                            <RetryRow
                              msg={entry.error}
                              onRetry={() => {
                                // Clear error then re-trigger after short delay to let queue settle
                                setCache(p => ({ ...p, [key]: { loading: false, error: null, text: null } }));
                                setTimeout(() => loadTabContent(verse, activeTab, true), 300);
                              }}
                            />
                          )}
                          {entry.text && (
